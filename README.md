...
For this project, we'll be building a Command Line app .

### Goals for the Command Line CRUD App

1. Build at least four models including a join model
2. Build out a CLI that allows users to interact with your models
...
2. Before you start building, take a look at the files you have available in this repo.
    * In the main directory, you've got a gemfile so remember to bundle install!
...
3. Your first goal should be to decide on your models and determine the relationships between them. You'll need one many-to-many relationship.
  Here are some ideas:
    ...
    * Tweet, Topic, Tweet topics: A tweet has many topics and a topic has many tweets, tweet_topic belongs to tweet and topic
    ...
    Whiteboard out your ideas and think about what attributes you'll want in the corresponding models. Where are instances stored in a many-to-many relationship?

### Data Models    
4. Make a new file for each model in your lib folder. What's the naming convention for a model filename? Check out previous labs for a reminder.

5. Be sure to include the relationships between your models.

### Relationships
6. Now is a great time top open up your console in the terminal and make sure everything's working properly. Your @@all variables are empty at this point, so start by creating a new instance of your model. For the train example, we'd do something like this:
```
fulton = Station.create(name: "Fulton")
```
You should see the station inserted into your app. Cool! Now let's test our relationships:
```
a = Line.create(name: "A")
fulton.lines << a
```
Woah! What did we just do there? The first part is simple: we added the a line to our line array. Then, we accessed our station, Fulton Station, and accessed its array of lines. (Because a station has many lines, right?) Finally, we pushed the A line into Fulton's lines. Amazing!! High fives all around.

![celebration](https://media.giphy.com/media/Is1O1TWV0LEJi/giphy.gif)

### Seeding the app
8. At this point, we could continue adding items to our program through the console, but let's be real. There are 425 train stations in New York--entering them individually would take forever!! There must be a better way...

Enter the seeds file. What is a seed file? It's a file, located in the db folder, where you create new instances of your classes and save them to your app. There are several ways this could happen. You could iterate over as csv file, for example, pulling out relevant data, and creating a new row in your app for every row in the file.
...
### CLI
9. Okay, so we've got our models; and we've got our relationships set up between them. Now, what do we do with all this stuff? We don't want our users to have to use the console every time they want to see which train lines go through fulton station, so let's create a command line interface!
...
10. Instead, let's create a Command Line Interface model in our lib directory. This model won't have a corresponding array (think @@all), it's just going to be a place for us to write methods relating to the interface of our app. Now, let's move the greet method definition into the Command Line Interface model.
...
First let's think big picture: our goal is to take the user's input--a string of a station name--and use that station name to find a station in our app. Then we want to grab all of that station's lines and output the lines to our user.

Now let's think about how we'll code out this process: first, we want a method that uses the station name to query our app, right? Know any methods we could write to find a station by its name?
...
Now, create a method that takes the user's input as an argument, and passes that variable to ```find_by```, returning the station from your app.
...
## Next Steps
  ...
  * Look for a <a href="http://web.mta.info/developers/data/nyct/subway/StationEntrances.csv">.csv file</a> or API that we can use to seed our app with lots and lots more data. How could you seed a app with rows from a csv file or with JSON data from an API?
  ...
  * Allow the user to do multiple searches without having to run the app each time. What if they want to search fifty different stations and then exit the app midway through a search?
